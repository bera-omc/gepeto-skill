...
Use this in **Refactor mode**. Run each of the 11 checks below against the agent the user shared (instructions file, knowledge base files, and any examples). Each check produces one finding. Group findings by severity using `refactor-output-template.md`.
...
- Use `refactor-output-template.md` to format the findings.
